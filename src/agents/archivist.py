...
class Archivist:
    ...
    def __init__(self, db_session: AsyncSession):
        ...
        self.item_repo = InteractableRepository(db_session)
        self.clue_repo = ClueDiscoveryRepository(db_session)
        self.knowledge_repo = KnowledgeRepository(db_session)
        self.system_repo = SessionRepository(db_session)
    ...
    async def get_location_view(self, entity_name: str) -> Dict[str, Any]:
        """获取指定实体当前视野的描述。"""
        # 找人
        entity = await self.entity_repo.get_by_name(entity_name)
        if not entity:
        ...
        location = await self.location_repo.get_by_id(entity.location_id)
        if not location:
    ...
    async def move_entity(self, entity_name: str, direction: str) -> Dict[str, Any]:
        ...
        current_loc = await self.location_repo.get_by_id(entity.location_id)
        ...
        target_loc = await self.location_repo.get_by_key(target_ref)
        
        # 如果没找到，再尝试当做中文名查找
        if not target_loc:
            target_loc = await self.location_repo.get_by_name(target_ref)
        ...
        await self.entity_repo.save(entity)
    ...
    async def travel_to_location(self, entity_name: str, target_ref: str) -> Dict[str, Any]:
        ...
        # 构建导航图
        raw_locs = await self.location_repo.get_navigation_graph_data()
